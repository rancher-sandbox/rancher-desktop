...
func reportCheckRegistration(w io.Writer, root string, locales []string) error {
	...
	if validatorData, err := os.ReadFile(validatorPath); err != nil {
		problems = append(problems, fmt.Sprintf("  cannot read settingsValidator.ts: %v", err))
	...
	if specData, err := os.ReadFile(specPath); err != nil {
		problems = append(problems, fmt.Sprintf("  cannot read settingsValidator.spec.ts: %v", err))
	} else {
	...
	if enKeys, err := loadYAMLFlat(translationsPath(root, "en-us.yaml")); err != nil {
		problems = append(problems, fmt.Sprintf("  cannot read en-us.yaml: %v", err))
	} else {
