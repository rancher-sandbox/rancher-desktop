...
async function ensure() {
  ...
  try {
    ...
    // Cant connect to the cluster so there is a problem.
    throw new Error(`Unable to connect to cluster ${e}`);
  }

  try {
    await Helm.status('homestead', 'cattle-system');
  } catch(e) {
    ...
    try {
      out = await Helm.install('homestead', './resources/homestead-0.0.1.tgz', 'cattle-system');
    } catch (e2) {
