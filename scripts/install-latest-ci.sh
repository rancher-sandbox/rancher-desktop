...
# 617701842                     1 file
# ---------------------------------------------------------------------------
ZIP=$(unzip -l "/tmp/$FILENAME" | perl -ne 'print if s/^\d.*Rancher/Rancher/')
if [ -z "$ZIP" ]; then
    echo "Cannot find inner archive in /tmp/$FILENAME"
    exit 1
fi

unzip -o "/tmp/$FILENAME" "$ZIP" -d /tmp

# Extract from inner archive into ~/Applications
APP="Rancher Desktop.app"
rm -rf "$HOME/Applications/$APP"
unzip -o "/tmp/$ZIP" "$APP/*" -d $HOME/Applications >/dev/null

download_artifact "bats.tar.gz"

# Despite the name, the downloaded bats.tar.gz is actually a ZIP file
unzip -o /tmp/bats.tar.gz bats.tar.gz -d /tmp

# Unpack bats into /tmp/bats
rm -rf /tmp/bats
mkdir /tmp/bats
tar xfz /tmp/bats.tar.gz -C /tmp/bats
