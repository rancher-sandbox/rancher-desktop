...
/**
 * Writes the manifest to disk in compound-entry form.  Always emits a
 * leading header comment so contributors editing the YAML directly see
 * the warning where they are editing; everything else round-trips
 * through `YAML.stringify`, which drops any other comments on the next
 * rddepman bump.
 */
...
# filenames; install-time downloads verify against the stored sha256.  To
# bump, run \`yarn rddepman <name>\`.  Hand-edits to a version must
# recompute the affected sha256 entries; stale digests fail postinstall
# verification.
`;
...
export async function fetchUpstreamChecksums(url: string, algorithm: 'sha256' | 'sha512'): Promise<Record<string, string>> {
  ...
  if (Object.keys(result).length === 0) {
    throw new Error(`${ url } parsed to zero ${ algorithm } checksum entries; verify the sidecar format.`);
  }
...
export function GlobalDependency<T extends abstract new(...args: any[]) => VersionedDependency>(Base: T) {
  abstract class GlobalDependency extends Base {
    ...
    async updateManifest(newVersion: Version, newChecksums: Record<string, Sha256Checksum>): Promise<Set<string>> {
      // Read from disk so a caller that just ran `git checkout` or otherwise
      // mutated the file outside this process sees the current state.  The
      // module cache reflects the snapshot at startup, which can drift.
