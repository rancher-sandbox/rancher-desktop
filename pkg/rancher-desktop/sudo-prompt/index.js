...
function MacApplet(instance, end) {
  const parent = Node.path.dirname(instance.path);
  ...
  Node.fs.mkdir(parent,
    (error) => {
      ...
      const zip = Node.path.join(parent, 'sudo-prompt-applet.zip');
...
function MacOpen(instance, end) {
  ...
  const binary = Node.path.join(instance.path, 'Contents', 'MacOS', 'applet');
...
function MacPropertyList(instance, end) {
  ...
  const plist = Node.path.join(instance.path, 'Contents', 'Info.plist');
...
function MacResult(instance, end) {
  const cwd = Node.path.join(instance.path, 'Contents', 'MacOS');
